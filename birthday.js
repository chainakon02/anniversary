...
const passcodeCard  = $('passcodeCard');
const passcodeSlots = Array.from(document.querySelectorAll('.passcode-slot'));
const passcodeHint  = $('passcodeHint');
...
/* ============================================================
   ACT 1 — PASSCODE UNLOCK (170826)
   ============================================================ */
const PASSCODE = '170826';

/* the target heart's beat — gentle, alive; paused when passcode is unlocked */
let beatTL = null;
function startBeat(){
...
/* a little burst of hearts + sparks when passcode unlocks */
...
function burstHearts(){
  ...
  for (let i = 0; i < 14; i++){
    ...
    el.className = 'burst';
    const s = heart ? rand(14, 24) : rand(5, 9);
  ...
  nodes.forEach(({ el, heart }) => {
    const ang = rand(-Math.PI * 1.1, 0.1);
    const dist = rand(heart ? 80 : 50, heart ? 210 : 130);
    gsap.to(el, {
      x: Math.cos(ang) * dist, y: Math.sin(ang) * dist - rand(15, 60),
      rotation: rand(-120, 120), scale: heart ? rand(0.8, 1.3) : rand(0.5, 1.1),
      duration: rand(0.7, 1.2), ease: 'power2.out',
    });
    gsap.to(el, { opacity: 0, duration: 0.5, delay: rand(0.35, 0.6), ease: 'power1.in', onComplete: () => el.remove() });
  });
}

/* --- unlock geometry: centers the flood exactly on the beating heart -------- */
function unlockGeom(){
  const tRect = target.getBoundingClientRect();
  const tcx = tRect.left + tRect.width / 2, tcy = tRect.top + tRect.height / 2;
  const impactX = tcx, impactY = tcy;
  const distC = Math.hypot(Math.max(impactX, W - impactX), Math.max(impactY, H - impactY));
  const reach = Math.hypot(W / 2, H / 2);
  return {
    fx: impactX - W / 2, fy: impactY - H / 2,
    floodScale: (distC * 1.15) / 70, bloomScale: (reach * 1.2) / 30,
  };
}
...
function buildFilm(m){
  ...
  // --- unlock celebration: passcode card and eyebrow fade, heart beats fast with joy ---
  t.to([eyebrow, passcodeCard], { opacity: 0, y: -12, duration: 0.35, ease: 'power2.inOut' }, 0)
   .to(target, { scale: 1.18, duration: 0.18, ease: 'power2.out' }, 0.06)
   .to(heartGlow, { scale: 1.4, opacity: 1, duration: 0.18, ease: 'power2.out' }, 0.06)
   .to(target, { scale: 1.0, duration: 0.15, ease: 'power2.in' }, 0.24)
   .add(burstHearts, 0.26)
   .to(target, { scale: 1.3, duration: 0.2, ease: 'power2.out' }, 0.39)
   .to(heartGlow, { scale: 1.6, opacity: 1, duration: 0.2, ease: 'power2.out' }, 0.39)
   .add(burstHearts, 0.55)
   .to(target, { scaleX: 1.6, scaleY: 1.6, duration: 0.4, ease: 'power2.in' }, 0.62)
   .to(heartGlow, { scale: 2.3, opacity: 0.9, duration: 0.38, ease: 'power2.in' }, 0.62)
...
/* --- passcode input management --------------------------------------------- */
const PRESETS = { 2: '0', 3: '8', 4: '2', 5: '6' };
let played = false;

function applyPresets(){
  Object.entries(PRESETS).forEach(([idx, val]) => {
    const slot = passcodeSlots[Number(idx)];
    if (slot) {
      slot.value = val;
      slot.readOnly = true;
      slot.tabIndex = -1;
      slot.classList.add('is-preset', 'is-filled');
    }
  });
}

function setupPasscode(){
  if (!passcodeSlots.length) return;
  applyPresets();

  passcodeSlots.forEach((slot, index) => {
    if (PRESETS[index] !== undefined) return; // slot 2, 3, 4, 5 are locked presets
    ...
    slot.addEventListener('input', () => {
      ...
      if (slot.value) {
        ...
        if (index === 0 && passcodeSlots[1]) {
          passcodeSlots[1].focus();
        }
      } else {
    ...
    slot.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Backspace') {
        if (index === 1) {
          if (!slot.value && passcodeSlots[0]) {
            e.preventDefault();
            passcodeSlots[0].value = '';
            passcodeSlots[0].classList.remove('is-filled');
            passcodeSlots[0].focus();
            clearError();
          } else if (slot.value) {
            slot.value = '';
            slot.classList.remove('is-filled');
      ...
      } else if (e.key === 'ArrowLeft' && index === 1 && passcodeSlots[0]) {
        passcodeSlots[0].focus();
      } else if (e.key === 'ArrowRight' && index === 0 && passcodeSlots[1]) {
        passcodeSlots[1].focus();
      }
    });
    ...
    slot.addEventListener('paste', (e) => {
      ...
      if (digits.length === 2) {
        passcodeSlots[0].value = digits[0];
        passcodeSlots[0].classList.add('is-filled');
        passcodeSlots[1].value = digits[1];
        passcodeSlots[1].classList.add('is-filled');
        passcodeSlots[1].focus();
        ...
        slot.value = digits[0];
        slot.classList.add('is-filled');
        if (index === 0 && passcodeSlots[1]) passcodeSlots[1].focus();
      }
      clearError();
      checkPasscode();
    });
...
function clearError(){
  if (passcodeError) {
    ...
    passcodeError.classList.remove('is-shown');
  }
  passcodeSlots.forEach(s => s.classList.remove('is-error'));
}

function checkPasscode(){
  const entered = passcodeSlots.map(s => s.value).join('');
...
function onPasscodeSuccess(){
  if (played) return;
  played = true;
  passcodeSlots.forEach(s => {
    s.classList.add('is-success');
    s.blur();
  });
  if (passcodeHint) passcodeHint.textContent = 'รหัสผ่านถูกต้องแล้วน้า 💖✨';
  stopBeat();
  cue('release'); cue('whoosh');
  filmTL = buildFilm(unlockGeom());
  filmTL.play(0);
}

function onPasscodeError(){
  [passcodeSlots[0], passcodeSlots[1]].forEach(s => s && s.classList.add('is-error'));
  if (passcodeError) {
    passcodeError.textContent = 'รหัสผ่านไม่ถูกต้องน้า ลองใหม่อีกครั้งนะ 🥺';
    passcodeError.classList.add('is-shown');
  }
  ...
  setTimeout(() => {
    if (played) return;
    [passcodeSlots[0], passcodeSlots[1]].forEach(s => {
      if (s) {
        s.value = '';
        s.classList.remove('is-filled', 'is-error');
      }
    });
    applyPresets();
    if (passcodeSlots[0]) passcodeSlots[0].focus();
  }, 700);
}

/* boot Act 1: reveal the target + passcode card, then start the beat */
function enter(){
  gsap.set(hero, { autoAlpha: 1 });
  gsap.set(eyebrow, { opacity: 0, y: 14 });
  gsap.set(target, { opacity: 0, y: 10, scaleX: 0.9, scaleY: 0.9 });
  gsap.set(heartGlow, { opacity: 0, scale: 1 });
  gsap.set(passcodeCard, { opacity: 0, y: 14 });
  ...
  [passcodeSlots[0], passcodeSlots[1]].forEach(s => {
    if (s) {
      s.value = '';
      s.classList.remove('is-filled', 'is-error', 'is-success');
    }
  });
  if (passcodeHint) passcodeHint.textContent = 'วันที่เท่าไหร่น๊าาา 💕';

  const tl = gsap.timeline({
    onComplete: () => {
      startBeat();
      if (passcodeSlots[0]) {
        try { passcodeSlots[0].focus({ preventScroll: true }); } catch (_) {}
      }
    }
  });
  ...
  tl.to(target,       { opacity: 1, y: 0, scaleX: 1, scaleY: 1, duration: 0.8, ease: 'power3.out' }, 0.1)
    ...
    .to(passcodeCard, { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' }, 0.4);
}
...
/* back to Act 1, ready to be unlocked again */
function resetAll(){
...
if (isRecord){
  window.bdayAPI = {
    start(){
      passcodeSlots.forEach((s, idx) => { s.value = PASSCODE[idx]; });
      onPasscodeSuccess();
    },
    replay(){ resetAll(); },
  };
...
/* ============================================================
   ACT 5 — GALLERY & SCROLL INTEGRATION
   ============================================================ */
...
const letterSection = $('letterSection') || $('gallerySection');
if (scrollCue && letterSection) {
  scrollCue.addEventListener('click', () => {
    letterSection.scrollIntoView({ behavior: 'smooth' });
  });
}

const sceneEl = $('scene') || document.querySelector('.scene');
